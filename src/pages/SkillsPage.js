    ...
    const renderedSkills = skillsByCategory.map((categorySkills, i) => {
        return (
            <div key={i} className="flex flex-col items-center md:items-start space-y-5 md:space-y-7">
                <h1 className="text-center text-xl md:text-2xl font-bold">{categorySkills[0].category}</h1>
    ...
    return (
        <div className="min-h-screen py-10 bg-blue-200 flex flex-col">
            <div className="text-center space-y-2 mb-10">
                ...
                <p className="text-lg">[Section sub-title]</p>
            </div>
            <div className="flex-grow flex flex-col md:flex-row items-center justify-center">
                <div className="md:hidden flex flex-row space-x-4 border-2 rounded-lg border-black w-fit p-2 mb-5">
                    {renderedSkillLevelColor}
                </div>
                <div className="flex flex-col space-y-9 md:space-y-11 mx-6">
                    {renderedSkills}
                </div>
                <div className="hidden md:flex flex-col space-y-2 border-2 rounded-lg border-black h-fit p-2">
                    {renderedSkillLevelColor}
                </div>
            </div>
